...
jar/exe/dmg package, or run "java -jar TITS-client.jar" to start the
...
desktop icons to start the TITS server.


Using the TITS-client.zip package:

Unpack the TITS-client.zip package and open a shell in the resulting TITS
...
the nux_db blockchain database. The start.sh script can be run from any
directory, it starts the java process in the background and uses ~/.nxt for
configuration files, logs, and the nux_db database. Unlike run.sh, start.sh
...
browser, without stopping the java process, and go to http://localhost:4268 ,
where the TITS UI should now be available.
...
to the TITS installation directory, as the use of special characters may result
...
values. You do not need to delete the defaults from TITS-default.properties, the
settings in TITS.properties override those in TITS-default.properties. This way,
when upgrading the software, you can safely overwrite TITS-default.properties
with the updated file from the new package, while your customizations remain
safe in the TITS.properties file.


How to contribute?

There are many ways to contribute to TITS. Here are some examples:
...
The TITS software is a client-server application. It consists of a java server
...
requests which other TITS client applications could use.

The blockchain is stored on disk using the H2 embedded database, inside the
nux_db directory. When upgrading, you should not delete the old nux_db
...
nux_db, except that it will take some extra time to download the blockchain
from scratch.

The default TITS client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file TITS.log, which gets
overwritten at restart. In case of an error, the TITS.log file may contain
helpful information, so include its contents when submitting a bug report.

In addition to the default user interface at http://localhost:4268 , the
following urls are available:

http://localhost:4268/test - a list of all available http API requests, very
...
http://localhost:4268/test?requestType=<specificRequestType> - same as above,
but only shows the form for the request type specified.

http://localhost:4268/doc - a javadoc documentation for client developers who
...
nxt.jar file using the enclosed jar.sh script, and then TITS.jar should be
